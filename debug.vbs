...
'MsgBox ("""" & QEMU_PATH & QEMU_EXE  & """ " & QEMU_OPTS & " -cpu Skylake-Client-v1 -serial stdio "& " -L . " & BIOS_OPT & " " & FW_FILE & " -hda fat:rw:image")
' Visual Studio QEMU debugging script.
'
' I like invoking vbs as much as anyone else, but we need to download and unzip our
' firmware file, as well as launch QEMU, and neither Powershell or a standard batch
...
'Call shell.Run("""" & QEMU_PATH & QEMU_EXE  & """ " & QEMU_OPTS & " -s -S "& " -L . " & BIOS_OPT & " " & FW_FILE & " -hda fat:rw:image" )
'MsgBox ("""" & QEMU_PATH & QEMU_EXE  & """ " & QEMU_OPTS & " -cpu Skylake-Client-v1 -serial stdio "& " -L . " & BIOS_OPT & " " & FW_FILE & " -hda fat:rw:image")
Call shell.Run("""" & QEMU_PATH & QEMU_EXE  & """ " & QEMU_OPTS & "    -cpu Skylake-Client-v1,vmx  -serial stdio "& " -L . " & BIOS_OPT & " " & FW_FILE & " -hda fat:rw:image" )

'Call shell.Run("""" & QEMU_PATH & QEMU_EXE  & " -windbg"" " & QEMU_OPTS & " -accel whpx -serial stdio "& " -L . " & BIOS_OPT & " " & FW_FILE & " -hda fat:rw:image" )

'Call shell.Run("code -g .vscode\launch.json", 0, False)
'Call shell.Run("code --folder-uri """ & fso.GetAbsolutePathName(".") & """ --command workbench.view.debug", 0, False)
